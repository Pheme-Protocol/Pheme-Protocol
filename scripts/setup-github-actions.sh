...
gh secret set DB_HOST --body "$(terraform output -raw db_endpoint)"
gh secret set DB_PORT --body "5432"
gh secret set DB_NAME --body "$(terraform output -raw db_name)"
gh secret set DB_USER --body "$(terraform output -raw db_username)"
...
gh api repos/{owner}/{repo}/branches/main/protection -X PUT -f \
  required_status_checks='{"strict":true,"contexts":["lint-test","solidity-scan","build","docker"]}' \
  enforce_admins=true \
